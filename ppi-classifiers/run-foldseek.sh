...
for monomer in "${unique_monomers[@]}"; do
  ...
  status=$(curl -s -L -w "%{http_code}" -o "fasta/${monomer}.fasta" "${uniprot_url}${monomer}.fasta")
  if [[ "${status}" -eq 200 ]]; then
    { # integrate multi-line sequences into a single line
        head -n1 "fasta/${monomer}.fasta"
        tail -n 2+ | paste -sd ''
    } > "fasta/${monomer}.fasta.tmp" && mv "fasta/${monomer}.fasta.tmp" "fasta/${monomer}.fasta"
...
# define some paths
FASTA_INPUT_PATH="${REPOSITORY_DIR}/output/fasta"
TSV_OUTPUT_PATH="${REPOSITORY_DIR}/output/tsv"
FOLDSEEK_INTERNAL_TEMP_DIR="transient/foldseek_internal_temp"

# run python script
python3 generate_hits.py \
  --fasta_input_dir "${DEFAULT_USER_BASE_DIR}/${FASTA_INPUT_PATH}" \
  --tsv_output_dir "${DEFAULT_USER_BASE_DIR}/${TSV_OUTPUT_PATH}" \
  --foldseek_internal_tmp_dir "${DEFAULT_USER_BASE_DIR}/${FOLDSEEK_INTERNAL_TEMP_DIR}" \
